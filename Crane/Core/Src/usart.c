...
***************************/
uint8_t USART2_RX_BUF[USART_REC_LEN]={0};  //鎺ユ敹缂撳啿,鏈?澶SART_REC_LEN涓瓧鑺?.
uint16_t USART2_RX_STA=0;      				 		 //鎺ユ敹鐘舵?佹爣璁?	
uint8_t aRxBuffer2[RXBUFFERSIZE];					 //HAL搴撲娇鐢ㄧ殑涓插彛鎺ユ敹缂撳啿
...
bit13~0锛?	鎺ユ敹鍒扮殑鏈夋晥瀛楄妭鏁扮洰
***************************/
uint8_t USART3_RX_BUF[USART_REC_LEN]={0};  //鎺ユ敹缂撳啿,鏈?澶SART_REC_LEN涓瓧鑺?.
uint16_t USART3_RX_STA=0;                  //鎺ユ敹鐘舵?佹爣璁?	
uint8_t aRxBuffer3[RXBUFFERSIZE]={0};      //HAL搴撲娇鐢ㄧ殑涓插彛鎺ユ敹缂撳啿
