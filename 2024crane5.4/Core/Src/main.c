...
/****串口接收全局数组****/
uint8_t DataBuff[50]={0};uint8_t DataBuff5[50]={0};
extern uint8_t RxBuff[1],RxBuff5[1];
...
//	HAL_UART_Transmit(&huart5, (uint8_t *)&lujing1, 1,0xFFFF);
...
//				stepmotor_move_rel(V_START,100,0.01,0.01,(2.0f)*SPR,STEPPER_MOTOR_2);;/* 一次加减速运动 */
				stepmotor_move_rel(V_START,80,0.01,0.01,(2.0f)*SPR,STEPPER_MOTOR_1);;/* 一次加减速运动 */
//				stepper_start(STEPPER_MOTOR_2);
				stepper_start(STEPPER_MOTOR_1);
				cs_flag=1;
		}
		else if(cs_flag==1&&g_motor1_sta==STATE_IDLE)
		{
				HAL_Delay(1000);
				g_add_pulse_count[0]=0;
//				g_add_pulse_count[1]=0;
//				stepmotor_move_rel(V_START,50,0.01,0.01,(-1.5f)*SPR,STEPPER_MOTOR_2);;/* 一次加减速运动 */
				stepmotor_move_rel(V_START,80,0.01,0.01,(-1.5f)*SPR,STEPPER_MOTOR_1);;/* 一次加减速运动 */
//				stepper_start(STEPPER_MOTOR_2);
				stepper_start(STEPPER_MOTOR_1);
				cs_flag=2;
		}
