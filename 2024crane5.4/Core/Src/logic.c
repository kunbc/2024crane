...
/******************************判断阶段**************************************/		
		...
		{
				if((ceju_flag==0 && location==5)||(ceju_flag==0 && location==3)) 
				{//识别20次
						for(int i=0; i<20; i++)
						{
							ceju();					
						...
						switch(cejvfankui_flag)//判断情况
						{
								case 2:
									...
									if(location==5)
									{
											HAL_UART_Transmit(&huart5, (uint8_t *)&path5_z, 1,0xFFFF);//都有，发送4 -> 径直走
									}
									else if(location==3)
									{
											HAL_UART_Transmit(&huart5, (uint8_t *)&path3_z, 1,0xFFFF);//都有，发送6 -> 径直走
									}
									servo1_zhuaqv=0; 
								...
								case 4:
									...
									if(location==5)
									{
											HAL_UART_Transmit(&huart5, (uint8_t *)&path5_j, 1,0xFFFF);////非都有，发送5 -> 规划路径
									}
									else if(location==3)
									{
											HAL_UART_Transmit(&huart5, (uint8_t *)&path3_j, 1,0xFFFF);////非都有，发送7 -> 规划路径
									}
									servo1_zhuaqv=0; 
									servo2_zhuaqv=0;printf("情况=%d\r\n",qingkuang); break;
...
/******************************工作阶段**************************************/
		if(qingkuang==1)//都在内环
		{
				if(gc_flag==0)
				{
					if (location==5||location==3)
					{
						if(sec2o3_zhuaqv()==OK)
						{
							gc_flag++;//1
							printf("取物完成\r\n");
							HAL_UART_Transmit(&huart5, (uint8_t *)&go, 1,0xFFFF);//抓完大车走		
						}
					}
				}
				else if(gc_flag==1)
				...
				else if(gc_flag==3)
				{
					...
					printf("大车走\r\n");
					Initflag++;	//7 //9
					HAL_UART_Transmit(&huart5, (uint8_t *)&go, 1,0xFFFF);//放完大车走
				}
		}
		else if(qingkuang==2)//都在外环
		{
				if(gc_flag==0)
				{
						if(nei_py()==OK)
						{
								printf("内平移完成\r\n");
								gc_flag++;//1		
								HAL_UART_Transmit(&huart5, (uint8_t *)&go, 1,0xFFFF);//大车走，等待抓取										
						}
				}
				else if(gc_flag==1)
				{
...
void sec1 (void)
{
	...
	if(zhengji_flag==1)
	{
		if(ceju_flag==0 && location==1) 
		{
			...
			if(servo1_zhuaqv==1) 			HAL_UART_Transmit(&huart5, (uint8_t *)&path1_z, 1,0xFFFF);//有，发送1, ->径直走
			else if(servo1_zhuaqv==2) HAL_UART_Transmit(&huart5, (uint8_t *)&path1_j, 1,0xFFFF);//无，发送2，->经过走
		}
...
void Crane_Init(void)
{
			...
			if(g_motor1_sta==STATE_IDLE && g_motor2_sta==STATE_IDLE)
			{
					...
					{
							stepper_start(STEPPER_MOTOR_3);
							HAL_UART_Transmit(&huart5, (uint8_t *)&go, 1,0xFFFF);//初始化完成，大车行进
