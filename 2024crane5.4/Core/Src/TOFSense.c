...
void panduan(uint8_t sta, uint8_t location)
{
	...
	{
			case fulwork:
					if(dis1<= 210)  			qvwu_infer[0][Location] = 2;//有物品
		 else if(dis1> 210)					qvwu_infer[0][Location] = 1;//没有物品
					if(dis2<= 205)				qvwu_infer[1][Location] = 2;//有物品
		 else if(dis2> 205)					qvwu_infer[1][Location] = 1;//没有物品
					break;
