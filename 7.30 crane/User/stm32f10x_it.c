...
void DEBUG_USART1_IRQHandler(void)
{
		...
		{
				USART_ClearITPendingBit(DEBUG_USART1,USART_IT_RXNE);//清除中断标志
		...
		if(count_1==2)
		{
				F4_Receive_Data(uctemp_1);//openmv数据处理函数
				count_1=0;
...
// 串口3中断服务函数
void DEBUG_USART3_IRQHandler(void)//串口全局中断服务函数
{
		if( USART_GetITStatus(DEBUG_USART3,USART_IT_RXNE)!=RESET )
		{
				USART_ClearITPendingBit(DEBUG_USART3,USART_IT_RXNE);//清除中断标志
				uctemp_3[count_3] = USART_ReceiveData(DEBUG_USART3);
				count_3++; 		
		}
		if(count_3==2)
		{
				F4_Receive_Data(uctemp_3);//openmv数据处理函数
				count_3=0;
//				printf("串口3工作\r\n");
		}
